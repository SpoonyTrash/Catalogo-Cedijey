...
test("the home page obtains products from the server composition root", () => {
    assert.match(homePageSource, /import\s+\{\s*connection\s*\}\s+from\s+["']next\/server["']/);
    assert.match(
        homePageSource,
        /import\s+\{\s*inventoryService\s*\}\s+from\s+["']@\/lib\/server\/inventory["']/,
    );
    assert.match(
        homePageSource,
        /await\s+connection\(\)[\s\S]*await\s+inventoryService\.getProducts\(\)/,
    );
});
